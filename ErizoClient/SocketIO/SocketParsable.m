...
- (ParseResult*)parseString:(NSString*) message {
    ...
    NSLog(@"currentIndex = %ld message.length = %ld message = %@",reader.currentIndex,reader.message.length,reader.message);
...
//            NSInteger value = [[reader read:1] integerValue];
//            if(value > 0) {
            NSString *vv = [reader read:1];
            if([self theStringIsNumbers:vv]) {
                idString = [idString stringByAppendingString:[NSString stringWithFormat:@"%@",vv]];
            } else {
                [reader advance:-2];
                break;
            }
    ...
    NSLog(@"dataArrayString = %@",dataArray);
...
- (void) parseSocketMessage:(NSString*)message {
    ...
    if (![self isStringEmpty:message] ) {
        ...
        NSLog(@"parseResult == %@ message = %@",parseResult,message);
        ...
        if(socketPacket){
            NSLog(@"socketPacket = %@ binary.class = %@ binary = %@",socketPacket,[binary class],binary);
            [self handlePacket:socketPacket];
        } else {
            NSLog(@"have no socketPacket");
...
/// 判断字符串是不是 由数字组成
- (BOOL)theStringIsNumbers:(NSString*)str {
