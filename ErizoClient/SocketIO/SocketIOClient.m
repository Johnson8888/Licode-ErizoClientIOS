...
- (SocketEngine*)addEngine{
    SocketEngine *engine = [[SocketEngine alloc] initWithOption:self
                                                            url:self.socketURL
                                                         config:self.config];
    _engine = engine;
    return _engine;
    ...
    OnAckCallback cb = ^(int timeout, AckCallback callback) {
        ...
        if(timeout != 0){
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeout * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                NSLog(@"timeout ack = %d items = %@",ack,items);
            });
...
-(OnAckCallback)emitWithAck:(NSString*) event items:(NSArray*) items {
    NSMutableArray *dataArray = [[NSMutableArray alloc] initWithObjects:event, nil];
    NSArray *ackArray = [dataArray arrayByAddingObjectsFromArray:items];
    return [self createOnAck:ackArray];
}
...
-(void)_emit:(NSArray*) data ack:(NSInteger)ack {
    dispatch_async(self.emitQueue,^{
        ...
        if ([[data description] containsString:@"publish"]) {
            NSLog(@"11 socketPacket data = %@ ack = %ld",data,(long)ack);
        }
...
#pragma mark SocketEngineClient
...
-(void)engineDidOpen:(NSString*)reason{
    NSLog(@"engineDidOpen reason %@", reason);
}


#pragma mark SocketIOClientSpec
-(void)handleAck:(NSInteger) ack data:(NSArray*) data{
    if( self.status != Connected ){
        return;
    }
    dispatch_async(self.handleQueue ,^{
        [self.ackHandlers executeAck:(int)ack items:data onQueue:self.handleQueue];
    });
}
    ...
    dispatch_async(self.handleQueue ,^{
        ...
        for( SocketEventHandler *handler in self.handlers ){
            if( [handler.event isEqualToString:event]){
                [handler executeCallback:data withAck:(int)withAck withSocket:self];
            }
        }
    });
}
