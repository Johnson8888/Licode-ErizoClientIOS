...
-(instancetype) initWithOption:(id<SocketEngineClient>)client
                           url:(NSURL*)url
    ...
    if(self) {
        ...
        for( NSString* key in config){
            if([@"path" isEqualToString:key]) {
                ...
                self.forcePolling = [config[key]boolValue];
            }else if ([@"forceWebsockets" isEqualToString:key]) {
                self.forceWebsockets = [config[key]boolValue];
            }
        }
        [self createURLs];
    }
...
-(void) createWebsocketAndConnect {
    /// by zhx
//    WebSocket* ws = [[WebSocket alloc] initWithURL:self.urlWebSocketWithSid
//                                         protocols:NULL];
    
    
    WebSocket* ws = [[WebSocket alloc] initWithURL:self.urlWebSocket
                                         protocols:NULL];
