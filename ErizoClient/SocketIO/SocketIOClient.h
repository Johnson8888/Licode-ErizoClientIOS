...
- (void)emitAck:(int)ack
           with:(nullable NSArray*) items;

- (instancetype _Nonnull)initWithSocketURL:(NSURL *_Nonnull)url
                                    config:(NSDictionary *_Nullable)config;

-(NSUUID* _Nullable) on:(NSString* _Nullable)event
                 callback:(NormalCallback) callback;


-(OnAckCallback)emitWithAck:(NSString*)event
                      items:(NSArray*)items;

-(void)emit:(NSString* _Nullable) event
